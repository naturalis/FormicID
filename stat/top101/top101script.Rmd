...
g1 <- ggplot(shotcount, aes(x=variable, y=value, fill=variable)) + 
    ...
    coord_cartesian(ylim = c(3350, 3425)) +
    scale_y_continuous(breaks=c(3350,3375,3400,3425)) +
...
caste_df$label_ypos <- c(602, 1160, 12, 1211, 642, 8457)
...
g2 <-
    ...
    labs(title = "Sum species images per shottype") +
    coord_flip() +
...
```{r}
image <-  ggdraw() + draw_image(exampledata, scale = 1)
final2 <- plot_grid(g1, caste_plot, nrow=2, labels=c("B", "C"), rel_heights = c(0.4,1))
final3 <- ggdraw() +
    draw_plot(g2, 0, 0, 1, 1) +
    draw_plot(final2, x=0.42, y=0.07, width=0.585, height=0.65) +
...
save_plot("species_shot_dist2.pdf",
          final3,
          base_width = 8.27,
          base_height = 11.69)
```
...
prefinal <- ggdraw() +
    draw_plot(g2, 0, 0, 1, 1) +
    draw_image(exampledata, -0.3, 0.12, scale = 0.55) +
    draw_plot(g1, 0.5, 0.45, 0.35, 0.25) +
    draw_plot_label(c("A", "C", "B                  Dataset examples"), c(0, 0.5, 0), c(1, 0.75, 0.92), size = 15)
title <-
    ggdraw() + draw_label("Formicidae image dataset (clean)",
                          size = 18,
                          fontface = 'bold')
